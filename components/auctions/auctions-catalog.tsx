...
import type { AuctionLot, AuctionSource, CatalogFilters, CatalogQuickTab } from "@/lib/auctions/types";
import {
  MILEAGE_PRESETS,
  REGION_LABELS,
  SOURCE_LABELS,
...
export function AuctionsCatalog({ lots }: Props) {
  ...
  const sources = useMemo(
    () =>
      (Object.keys(SOURCE_LABELS) as AuctionSource[]).filter((s) =>
        lots.some((l) => l.source === s),
      ),
    [lots],
  );
    ...
    <div className="space-y-4">
      ...
      {sources.length > 1 ? (
        <Field label="Площадка">
          <select
            value={f.source}
            onChange={(e) => patch({ source: e.target.value as AuctionSource | "" })}
            className={fieldClass}
          >
  ...
  return (
    <div className="catalog-layout">
      ...
      <div ref={resultsRef} className="catalog-results min-w-0 flex-1">
        ...
        <LotGrid key={filterKey} lots={visible} />
