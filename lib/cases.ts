...
function lotToCase(lot: AuctionLot): ClientCase | null {
  ...
  const market = Math.round(turnkey.amount * markup);
  ...
  return {
    ...
    budget: turnkey.amount,
    market,
    currency: turnkey.currency,
    days,
    story: `${damage}. Пробег ${lot.odometer.toLocaleString("ru-RU")} ${lot.odometerUnit}. Расчёт под ключ: аукцион + доставка + разборка.`,
    result: turnkey.label,
